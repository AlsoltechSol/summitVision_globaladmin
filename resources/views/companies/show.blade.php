<div class="modal-body">
    <div class="row">
        <div class="col-md-12">
            ...
            <p><strong>Server Config Status:</strong> {{ $company->server_config_status == 1 ? 'Success' : 'Failed'}}</p>
        ...
        <div class="col-md-12">
            <p><strong>Create Domain and Dir:</strong> {{ $company->create_domain_and_dir == 1 ? 'Success' : 'Failed'  }}</p>
            <p><strong>Database Create and Config:</strong> {{ $company->database_create_and_config == 1 ? 'Success' : 'Failed' }}</p>
            <p><strong>Fileop:</strong> {{ $company->fileop == 1 ? 'Success' : 'Failed' }}</p>
            <p><strong>Modify Env:</strong> {{ $company->modify_env == 1 ? 'Success' : 'Failed'  }}</p>
            <p><strong>Plan:</strong> {{ $company->plan }}</p>
            <p><strong>Plan Expire Date:</strong> {{ $company->plan_expire_date }}</p>
            <p><strong>Company Name:</strong> {{ $company->company_name }}</p>
            <p><strong>Setup by Cron:</strong> {{ $company->setup_by_cron > 1 ? 'Success' : ( $company->setup_by_cron == null ? 'Never Executed' : 'Failed' )  }}</p>
